...
namespace Tesseract
{
    ...
    public class TesseractEngine : DisposableBase
    {
        ...
        public Page Process(Pix image, string inputName, Rect region, PageSegMode? pageSegMode = null)
        {
            ...
            if (!string.IsNullOrEmpty(inputName)) TessApi.Native.BaseApiSetInputName(handle, inputName);
        ...
        private string GetTessDataPrefix()
        {
            ...
            catch (SecurityException e)
            {
                trace.TraceEvent(TraceEventType.Error, 0,
                    "Failed to detect if the environment variable 'TESSDATA_PREFIX' is set: {0}", e.Message);
                return null;
            }
        }
