    ...
    public class PixArray : DisposableBase, IEnumerable<Pix>
    {
        ...
        private class PixArrayEnumerator : DisposableBase, IEnumerator<Pix>
        {
            ...
            public bool MoveNext()
            {
                ...
                {
                    if (items[index] == null) items[index] = array.GetPix(index);
                    current = items[index];
                    index++;
                    return true;
                }
        ...
        {
            Guard.Require("index", index >= 0 && index < Count, "The index {0} must be between 0 and {1}.", index,
                Count);
        ...
        {
            Guard.Require("accessType", accessType == PixArrayAccessType.Clone || accessType == PixArrayAccessType.Copy,
