...
namespace Tesseract
{
    public sealed unsafe class Pix : DisposableBase, IEquatable<Pix>
    {
        ...
        public Pix Clone()
        {
            var clonedHandle = LeptonicaApi.Native.pixClone(handle);
        ...
        {
            var result = LeptonicaApi.Native.pixScale(handle, scaleX, scaleY);
        ...
        private Pix(IntPtr handle)
        {
            ...
            if (colorMapHandle != IntPtr.Zero) colormap = new PixColormap(colorMapHandle);
        }
        ...
        public PixColormap Colormap
        {
            ...
            set
            {
                if (value != null)
                {
                    if (LeptonicaApi.Native.pixSetColormap(handle, value.Handle) == 0) colormap = value;
                }
                else
                {
                    if (LeptonicaApi.Native.pixDestroyColormap(handle) == 0) colormap = null;
                }
            }
        }
        ...
        public bool Equals(Pix other)
        {
            ...
            int same;
            if (LeptonicaApi.Native.pixEqual(Handle, other.Handle, out same) != 0)
        ...
        public Pix BinarizeOtsuAdaptiveThreshold(int sx, int sy, int smoothx, int smoothy, float scorefract)
        {
            ...
            IntPtr ppixth, ppixd;
            var result = LeptonicaApi.Native.pixOtsuAdaptiveThreshold(handle, sx, sy, smoothx, smoothy, scorefract,
                out ppixth, out ppixd);
        ...
        public Pix BinarizeSauvola(int whsize, float factor, bool addborder)
        {
            ...
            Guard.Require("whsize", whsize < maxWhSize,
            ...
            var result = LeptonicaApi.Native.pixSauvolaBinarize(handle, whsize, factor, addborder ? 1 : 0, out ppixm,
                out ppixsd, out ppixth, out ppixd);
        ...
        public Pix BinarizeSauvolaTiled(int whsize, float factor, int nx, int ny)
        {
            ...
            IntPtr ppixth, ppixd;
            var result =
                LeptonicaApi.Native.pixSauvolaBinarizeTiled(handle, whsize, factor, nx, ny, out ppixth, out ppixd);
        ...
        public Pix ConvertRGBToGray(float rwt, float gwt, float bwt)
        {
            ...
            var resultPixHandle = LeptonicaApi.Native.pixConvertRGBToGray(handle, rwt, gwt, bwt);
        ...
        {
            float pAngle, pConf;
            var resultPixHandle = LeptonicaApi.Native.pixDeskewGeneral(handle, sweep.Reduction, sweep.Range,
                sweep.Delta, redSearch, thresh, out pAngle, out pConf);
        ...
        {
            var resultHandle = LeptonicaApi.Native.pixRotate90(handle, direction);
